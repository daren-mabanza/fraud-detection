...
def calcul_distances(
    ...
    """
    Calcule la distance entre deux paires (lat, lon) ligne à ligne
    et ajoute le résultat dans une nouvelle colonne du DataFrame.
    ...
    pts1 = df[[lat1_col, lon1_col]].to_numpy()
    pts2 = df[[lat2_col, lon2_col]].to_numpy()

    df[out_col] = haversine_vector(pts1, pts2, unit=u)
    return df
