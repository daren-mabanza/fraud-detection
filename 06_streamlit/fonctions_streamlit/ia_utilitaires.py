...
def build_prompt_for_transaction(proba, input_dict, top_features):
    ...
    feature_mapping = {
        ...
        "montant_transaction": "le montant de la transaction",
    ...
    for name, shap_value in top_features[:5]:
        clean_name = str(name).split("_scaler_")[-1].strip()
    ...
    contexte_decision = (
        ...
        f"Le seuil de blocage est fixé à {seuil_pct}. "
...
Ton explication doit donc rester strictement centrée sur les régularités apprises dans ces données simulées.
Une transaction pouvant paraître étrange dans la réalité ne doit pas être décrite comme objectivement suspecte si ce n'est pas ce que montrent les facteurs fournis ici.

TA MISSION :

Explique la décision au client en 3 à 4 phrases maximum.
...
Cite toujours des éléments concrets comme le montant, l'heure et le type de commerce.
